 ...
 *
 * Example:
 * [cursorTarget] call kat_aceExposure_fnc_SAMLocal;
 *
 * Public: No
 */

params ["_player", "_target", "_damage", "_source", "_projectile", "_hitIndex", "_instigator", "_hitPoint"];
...
{
	private _path = (configFile >> "ACE_Medical_Advanced" >> "Injuries" >> "fractures" >> _x);
	private _selections = getArray (_path >> "selections");
	private _cause = getArray (_path >> "causes");
	if (_selection in _selections && _source in _cause) then {
